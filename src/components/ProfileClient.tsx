...
const ProfileClient: React.FC = () => {
  ...
  const [giverScore, setGiverScore] = useState<number>(0);
  ...
  useEffect(() => {
    ...
    const fetchUserData = async () => {
      try {
        ...
        try {
          // ユーザーのアクティビティとスコアを取得
          const response = await fetch(`/api/activities/user/${MOCK_USER.id}`);
  ...
  const addMockActivity = async (activityType: string) => {
    try {
      ...
      if (data.success) {
        // アクティビティ追加後にデータを再取得
        const refreshResponse = await fetch(`/api/activities/user/${MOCK_USER.id}`);
    ...
    <div className="profile-page max-w-4xl mx-auto p-4 md:p-8">
      <div className="profile-header bg-white rounded-lg shadow-md p-6 mb-6 flex flex-col md:flex-row items-center">
        ...
        <div className="profile-info flex-1">
          <h1 className="text-2xl font-bold">{MOCK_USER.name}さんのプロフィール</h1>
          <p className="text-gray-600 mb-2">{MOCK_USER.email}</p>
          <p className="text-sm text-gray-500">ユーザーID: {MOCK_USER.id}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="sidebar">
          <GiverScoreDisplay score={giverScore} />
