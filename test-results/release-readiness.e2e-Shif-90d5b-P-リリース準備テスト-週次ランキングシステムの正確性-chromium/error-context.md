...
- Name: ShiftWith MVP リリース準備テスト >> 週次ランキングシステムの正確性
- Location: /Volumes/Samsung/Works/personality-quiz/tests/e2e/release-readiness.e2e.test.ts:221:7
...
Call log:
  - waiting for locator('[data-testid="weekly-ranking"]')

    at /Volumes/Samsung/Works/personality-quiz/tests/e2e/release-readiness.e2e.test.ts:227:16
```
...
> 227 |     await page.click('[data-testid="weekly-ranking"]');
  ...
  296 |   await page.goto('/auth/login');
  297 |   await page.fill('[data-testid="email-input"]', email);
  298 |   await page.fill('[data-testid="password-input"]', password);
  299 |   await page.click('[data-testid="login-submit"]');
  300 |   await page.waitForURL('/dashboard');
  301 | } 
```